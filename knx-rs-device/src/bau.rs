...
impl Bau {
    ...
    fn process_control_tpdu(
        &mut self,
        frame: &CemiFrame,
        tpdu_type: knx_rs_core::message::TpduType,
        now_ms: u64,
    ) {
    ...
    fn queue_control_frame(
        ...
        let src = self.individual_address();
        let dst = DestinationAddress::Individual(destination);
        let payload = knx_rs_core::tpdu::encode_control(tpdu_type, seq_no);
        self.outbox.push_back(CemiFrame::new_l_data(
            MessageCode::LDataReq,
            src,
            dst,
    ...
    fn queue_data_connected_frame(
        ...
        let src = self.individual_address();
        let dst = DestinationAddress::Individual(destination);
        let payload = knx_rs_core::tpdu::encode_data_connected(seq_no, apdu);
        self.outbox.push_back(CemiFrame::new_l_data(
            MessageCode::LDataReq,
            src,
            dst,
